...
void __attribute__((noreturn)) bad_trap(uintptr_t* regs, uintptr_t dummy, uintptr_t mepc)
{
    int cause = read_csr(mcause);
        printm("[exception mcause %d, mepc %p ]",read_csr(mcause),read_csr(mepc));
        switch(cause)
        {
        case 0:		printm("Instruction address misaligned \n"); break;
        case 1:		printm("Instruction access fault \n"); break;
        case 2:		printm("Illegal instruction \n"); break;
        case 3:		printm("Breakpoint \n"); break;
        case 4:		printm("Load address misaligned \n"); break;
        case 5:		printm("Load access fault \n"); break;
        case 6:		printm("Store/AMO address misaligned \n"); break;
        case 7:		printm("Store/AMO access fault \n"); break;
        case 8:		printm("Enviorment call from U-mode \n"); break;
        case 9:		printm("Enviorment call from S-mode \n"); break;
        case 10:	printm("Reserved Exception \n"); break;
        case 11:	printm("Enviorment call from M-mode\n"); break;
        case 12:	printm("Instruction page fault \n"); break;
        case 13:	printm("Load page fault \n"); break;
        case 14:	printm("Reserved Exception \n"); break;
        case 15:	printm("Store/AMO page fault \n"); break;
        default:	printm("Reserved Exception"); break;
        }
        die("\n");
        //while(1);
...
void trap_ebreak(uintptr_t* regs, uintptr_t dummy, uintptr_t mepc)
{
    //die("machine mode: unhandlable trap  trap_ebreak %d @ %p", read_csr(mcause), mepc);
    //if(mepc==3)
...
void mcall_trap(uintptr_t* regs, uintptr_t mcause, uintptr_t mepc)
{
  ...
  uintptr_t n = regs[17], arg0 = regs[10], arg1 = regs[11], retval, ipi_type;
      ...
      uintptr_t _mstatus = read_csr(mstatus);
      _mstatus = INSERT_FIELD(_mstatus, 0x80, 1);
  ...
  case 11:
  {
      ...
      //mstatus = INSERT_FIELD(mstatus, 0x100, 0);
...
void redirect_exception(uintptr_t epc, uintptr_t mstatus, uintptr_t badaddr)
{
  write_csr(sbadaddr, badaddr);
  write_csr(sepc, epc);
  write_csr(scause, read_csr(mcause));
  write_csr(mepc, read_csr(stvec));
  write_csr(mbadaddr, 0);
...
void mem_fault_trap(uintptr_t* regs, uintptr_t mcause, uintptr_t mepc)
{
  redirect_exception(mepc, read_csr(mstatus), read_csr(mbadaddr));
}
